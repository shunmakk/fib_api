...
```
##### テストサーバーの起動と停止
beforeAllフックでテストサーバーを起動し、afterAllフックでテストサーバーを停止する。これにより、非同期操作がテスト終了後に続かないようになります。
##### ユニットテストの実行
...
### なぜRebderにhostingした？
herokuと比較検討した際、renderは無料でherokuは有料だったため。しかしCPU性能やセッションの継続時間は負けているため大規模なアプリケーションをデプロイする場合はherokuの方がいいかもです。
